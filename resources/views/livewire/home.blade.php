        ...
        <div class="grid grid-cols-12 gap-2 items-center">
            ...
            <div class="col-span-8 flex justify-center px-2">
                <input type="text" placeholder="Search"
                       class="border-0 outline-none focus:outline-none w-full bg-gray-100 rounded-lg focus:ring-0 hover:ring-0"/>
            </div>
    ...
    <main class="grid lg:grid-cols-12 gap-8 md:mt-10">
                    ...
                    @for($i=0; $i < 20; $i++)
                        <li class="flex flex-col justify-center w-20 gap-1 p-2">
                            <x-avatar story src="https://source.unsplash.com/500x500?face-{{$i}}"
                                      class="w-14 h-14"></x-avatar>
                            <p class="text-sm font-medium truncate">{{ fake()->name }}</p>
                        </li>
                    @endfor
                </ul>
            </section>
        ...
        <aside class="lg:col-span-4 border hidden lg:block p-4">
            <div class="flex items-center gap-2">
                <x-avatar story src="https://source.unsplash.com/500x500?face" class="w-12 h-12"></x-avatar>
                ...
                <ul class="my-2 space-y-3">
                    @for($i=0; $i < 8; $i++)
                        <li class="flex items-center gap-3">
                            <x-avatar story src="https://source.unsplash.com/500x500?face-{{$i}}"
                                      class="w-12 h-12"></x-avatar>

                            <div class="grid grid-cols-7 w-full gap-2">
                                <div class="col-span-5">
                                    <h5 class="font-semibold text-sm truncate">{{ fake()->name }}</h5>
